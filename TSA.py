...
class TSA(nn.Module):
    ...
    def forward(self, x, time_W_U_params, time_W_V_params, time_W_Z_params):
        """
        :param x: B,T,d_model
        :param time_W_U_params: B, T, d_model * n_heads * d_k
        :param time_W_V_params: B, T, d_model * n_heads * d_k
        :param time_W_Z_params: B, T, d_model * n_heads * d_v
        :return:
        """
        batch_size, num_of_hour, d_model = x.shape
        w_Q = time_W_U_params.reshape(batch_size, num_of_hour, d_model, self.n_heads * self.d_k)
        w_K = time_W_V_params.reshape(batch_size, num_of_hour, d_model, self.n_heads * self.d_k)
        w_V = time_W_Z_params.reshape(batch_size, num_of_hour, d_model, self.n_heads * self.d_v)
        # B, H, T, d_k
        input_Q = torch.einsum('b t d,b t d h->b t h', x, w_Q).reshape(batch_size, num_of_hour, self.n_heads,
                                                                       self.d_k).transpose(1, 2)
        input_K = torch.einsum('b t d,b t d h->b t h', x, w_K).reshape(batch_size, num_of_hour, self.n_heads,
                                                                       self.d_k).transpose(1, 2)
        input_V = torch.einsum('b t d,b t d h->b t h', x, w_V).reshape(batch_size, num_of_hour, self.n_heads,
                                                                       self.d_v).transpose(1, 2)
        ...
        # B, F, N, T
        output = self.linear(output).transpose(1, 2).unsqueeze(1)
        return output
...
class create_model(nn.Module):
    ...
    def forward(self, x, time_W_U_params, time_W_V_params, time_W_Z_params, sp_W_Q_params, sp_W_K_params,
        ...
        """
        :param x: B,F,N,T
        :param time_U_params:B, T, d_model * n_heads * d_k
        :param time_V_params:B, T, d_model * n_heads * d_k
        :param time_Z_params:B, T, d_model * n_heads * d_v
        :param sp_W_Q_params:B, N, d_model * n_heads * d_k
        :param sp_W_K_params:B, N, d_model * n_heads * d_k
        :param sp_W_V_params:B, N, d_model * n_heads * d_v
        :return: B, F_out, N, T
