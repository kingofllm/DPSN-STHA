...
class GAU(nn.Module):
    ...
    def forward(self, x, time_W_U_params, time_W_V_params, time_W_Z_params):
        ...
        time_W_V_params = time_W_V_params.reshape(batch_size, d_model, self.time_features_out)
        time_W_U_params = time_W_U_params.reshape(batch_size, d_model, self.time_features_out)
        time_W_Z_params = time_W_Z_params.reshape(batch_size, d_model, self.time_features_out)

        v = torch.einsum('b t d, b d o->b t o', x, time_W_V_params)  # B, T, O
        gate = torch.einsum('b t d, b d o->b t o', x, time_W_U_params)  # B, T, O
        v = self.SiLU(v)
        gate = self.SiLU(gate)

        z = self.SiLU(torch.einsum('b t d, b d o->b t o', x, time_W_Z_params))  # B, T, O
...
class create_model(nn.Module):
    def __init__(self, d_model, nodes, num_of_hour, num_of_features, d_k, d_v, n_heads, features_out,
        ...
        self.ST_conv = nn.Conv2d(2, features_out, kernel_size=1)
    ...
    def forward(self, x, time_W_U_params, time_W_V_params, time_W_Z_params, sp_W_Q_params, sp_W_K_params,
        ...
        STF = torch.cat([GAU, SAT], dim=1)  # B, 2F, N, T
        STF = self.ST_conv(STF)  # B, F_out, N, T
